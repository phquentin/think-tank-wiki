...
└── wiki/                 ← agent-maintained, user-driven (edits via workflows, not by hand)
    ...
    ├── fundamentals.md   ← primer on recurring terms (grows on-demand)
    ├── index.md          ← content catalog
    ├── log.md            ← chronological history
    ├── open-questions.md ← derived catalog of [open-question] markers (regenerated by /lint)
    ├── todos.md          ← derived catalog of [TODO] markers (regenerated by /lint)
    ├── entities/         ← concrete named things: companies, people, papers, regulations
    ├── concepts/         ← abstract topics
    ├── sources/          ← per-source summary pages produced by /ingest
    ├── synthesis/        ← /query answers and analytic write-ups filed back
    └── decisions/        ← user-set targets/decisions via /decide
```

(`.gitignore` and the gitignored `.obsidian/` vault state are omitted above.)
